...
async function updateDictType(id: number | string, params: Recordable<any>) {
  params.id = id;
  return requestClient.put(`/${sysApi}/sys/dict/type`, params);
}
...
async function deleteDictType(id: string) {
  return requestClient.delete(`/${sysApi}/sys/dict/type`, [id]);
}
/**
 * 删除字典数据
 * @param id 字典数据ID
...
async function deleteDictData(id: string) {
  return requestClient.delete(`/${sysApi}/sys/dict/data`, [id]);
}
...
async function updateDictData(id: number | string, params: Recordable<any>) {
  params.id = id;
  return requestClient.put(`/${sysApi}sys/dict/data`, params);
}
