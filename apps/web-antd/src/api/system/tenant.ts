...
async function createTenant(data: Omit<SystemTenantApi.SystemTenant, 'id'>) {
  return requestClient.post(`/${sysApi}/tenant/member`, data);
}
...
async function updateTenant(
  ...
  data.id = id;
  return requestClient.put(`/${sysApi}/tenant/member`, data);
}
...
async function deleteTenant(id: string) {
  return requestClient.delete(`/${sysApi}/tenant/member/${id}`);
}
